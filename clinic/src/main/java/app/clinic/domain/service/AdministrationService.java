...
public class AdministrationService {
    ...
    public void administerMedication(String orderNumber, int item) {
        ...
        MedicationOrder medOrder = order.getMedications().stream().filter(m -> m.getItem() == item).findFirst().orElseThrow(() -> new IllegalArgumentException("Medication item not found in order"));
    ...
    public void administerProcedure(String orderNumber, int item) {
        Order order = orderRepository.findByOrderNumber(new OrderNumber(orderNumber)).orElseThrow(() -> new IllegalArgumentException("Order not found"));
        ProcedureOrder procOrder = order.getProcedures().stream().filter(p -> p.getItem() == item).findFirst().orElseThrow(() -> new IllegalArgumentException("Procedure item not found in order"));
